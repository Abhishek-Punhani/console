...
export function ComplianceDrillDown({ data }: Props) {
  ...
  const [profileFilter, setProfileFilter] = useState<string>(initialProfile);
  ...
  const uniqueClusters = useMemo(
    () => [...new Set(allRows.map((r) => r.cluster))].sort(),
    [allRows],
  );
  const uniqueProfiles = useMemo(
    () => [...new Set(allRows.map((r) => r.profile).filter(Boolean))].sort(),
    [allRows],
  );
  ...
  const otherCount = filteredRows.filter(
    (r) => r.status === "other" || r.status === "not-applicable",
  ).length;
  const activeFilters = [
    statusFilter,
    severityFilter,
    ...
    <div className="flex flex-col h-full -m-6">
      ...
      <div className="px-6 pt-6 pb-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
          ...
          <button
            onClick={() => {
              setStatusFilter(statusFilter === "pass" ? "" : "pass");
              resetPage();
            }}
            className={cn(
              "p-3 rounded-lg border transition-colors text-left",
          ...
          <button
            onClick={() => {
              setStatusFilter(statusFilter === "fail" ? "" : "fail");
              resetPage();
            }}
            className={cn(
              "p-3 rounded-lg border transition-colors text-left",
          ...
          <button
            onClick={() => {
              setStatusFilter(statusFilter === "other" ? "" : "other");
              resetPage();
            }}
            className={cn(
              "p-3 rounded-lg border transition-colors text-left",
          ...
          <div className="rounded-lg border border-border overflow-hidden">
            ...
            <div className="grid grid-cols-[1fr_2fr_100px_100px_120px_120px] gap-px bg-border text-xs font-medium text-muted-foreground">
              <button
                ...
                className="flex items-center gap-1 px-3 py-2 bg-card/80 hover:bg-card transition-colors"
              >
            ...
            {pagedRows.map((row, i) => (
              ...
              >
                ...
                <div
                  className="px-3 py-2.5 text-xs text-muted-foreground truncate"
        ...
        <div className="px-6 py-3 border-t border-border">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {uniqueClusters.map((cluster) => {
              const clusterRows = filteredRows.filter(
                (r) => r.cluster === cluster,
              );
              const cPass = clusterRows.filter(
                (r) => r.status === "pass",
              ).length;
              const cFail = clusterRows.filter(
                (r) => r.status === "fail",
              ).length;
              const cTotal = clusterRows.length;
              const cScore =
                cTotal > 0 ? Math.round((cPass / cTotal) * 100) : 0;
              return (
                <button
                  key={cluster}
                  onClick={() => {
                    setClusterFilter(clusterFilter === cluster ? "" : cluster);
                    resetPage();
                  }}
                  className={cn(
