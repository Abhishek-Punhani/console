...
import type {
  TrestleClusterStatus,
  OscalProfile,
} from "../../hooks/useTrestle";
...
function makeClusterStatus(
  overrides: Partial<TrestleClusterStatus> = {},
): TrestleClusterStatus {
  return {
    cluster: "cluster-1",
    installed: true,
...
function setTrestleReturn(
  overrides: Partial<ReturnType<typeof useTrestle>> = {},
) {
...
describe("TrestleScan", () => {
  ...
  describe("not installed state", () => {
    it("renders install prompt when not installed and not demo data", () => {
      ...
      render(<TrestleScan />);
      expect(
        screen.getByText("Compliance Trestle (CNCF Sandbox)"),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Compliance-as-code using NIST OSCAL/),
      ).toBeInTheDocument();
  ...
  describe("degraded state", () => {
    it("renders degraded warning when installed clusters have totalControls=0", () => {
      ...
      render(<TrestleScan />);
      expect(
        screen.getByText(/Trestle Installed — No Assessments/),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/no OSCAL assessment results/),
      ).toBeInTheDocument();
    });
  ...
  describe("healthy render state", () => {
    it("renders overall score, score label, and pass/fail/other counters", () => {
      ...
      const mainScore = scoreElements.find((el) =>
    ...
    it("clicking total controls opens compliance drilldown", async () => {
      ...
      render(<TrestleScan />);
      const allControlsButtons = screen.getAllByTitle(
  ...
  describe("profile interaction", () => {
    ...
    it("expanded pass/fail/other chips open the same compliance dialog with profile + status", async () => {
      const user = userEvent.setup();
      setTrestleReturn();
      ...
      await user.click(
        screen.getByTitle("View passing controls for NIST 800-53 rev5"),
      );
      expect(mockDrillToCompliance).toHaveBeenCalledWith("pass", {
        profile: "NIST 800-53 rev5",
      });

      await user.click(
        screen.getByTitle("View failing controls for NIST 800-53 rev5"),
      );
      expect(mockDrillToCompliance).toHaveBeenCalledWith("fail", {
        profile: "NIST 800-53 rev5",
      });

      await user.click(
        screen.getByTitle("View other controls for NIST 800-53 rev5"),
      );
      expect(mockDrillToCompliance).toHaveBeenCalledWith("other", {
        profile: "NIST 800-53 rev5",
      });
    });
  });
  ...
  describe("edge cases", () => {
    ...
    it("does not show per-cluster section when only one installed cluster", () => {
      ...
      render(<TrestleScan />);
      expect(
        screen.queryByText("Per-cluster compliance"),
    ...
    it("renders 0% score correctly", () => {
      ...
      expect(
        scores.find((el) => el.className.includes("text-3xl")),
      ).toBeTruthy();
      expect(screen.getByText("Critical")).toBeInTheDocument();
    });
